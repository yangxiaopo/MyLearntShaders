    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // Invert
                float3 tmp = 1.0f - tex.aaa;
                float4 invertedColors = float4(tmp, 1.0f);

                // Replace color
                float dis = distance(float4(1,1,1,1), invertedColors.x);
                float4 colorMask = lerp(float4(1,1,1,1), invertedColors.x, saturate((dis - 1) / 1));
